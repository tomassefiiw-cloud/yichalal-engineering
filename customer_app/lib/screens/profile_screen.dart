    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(20), children: [
        Center(child: Column(children: [
          const CircleAvatar(radius: 40, backgroundColor: AppColors.orangeLight, child: Icon(Icons.person, size: 38, color: AppColors.orangeDark)),
        ...
        const SizedBox(height: 12),
        TextField(controller: _address, maxLines: 2, decoration: const InputDecoration(labelText: 'Address', prefixIcon: Icon(Icons.location_on_outlined))),
        const SizedBox(height: 16),
        ElevatedButton.icon(
          onPressed: () async {
            await Repo.instance.upsertUser(user.copyWith(fullName: _name.text.trim(), email: _email.text.trim().isEmpty ? null : _email.text.trim(), address: _address.text.trim()));
            await auth.refresh();
